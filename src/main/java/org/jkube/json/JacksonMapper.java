...
public class JacksonMapper implements Object2StringMapper {
	...
	JacksonMapper()
		...
		log("Initialized Jackson ObjectMapper");
	}

	@Override
	public <R> R string2Object(final String jsonString, final Class<R> objectClass) {
		return onException(() -> (R)readMethod.invoke(mapper, jsonString, objectClass)).rethrow("Could not parse Json");
	}

	@Override
	public String object2String(final Object object) {
		return onException(() -> (String)writeMethod.invoke(mapper, object)).rethrow("Could not generate Json");
	}
