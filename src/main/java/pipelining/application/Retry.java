...
import pipelining.logging.exception.ThrowingRunnable;
...
public class Retry {
		...
		while(!done) {
			...
			if (retryable.hasSucceeded()) {
				...
				if (retryable.getException() != null) {
					...
					if (reason == null) {
						Log.warn("Unspecified failure occurred in trial #{}", trial);
					} else {
						Log.warn("Failure occurred in trial #{}: {}", trial, reason);
					}
				}
				if (retryable.shouldRetry()) {
					if (trial <= maxNumRetries) {
						long thisWait = wait;
						Log.debug("Sleeping for {}ms.", thisWait);
						Log.interruptable(() -> Thread.sleep(thisWait));
						wait = Math.round(wait * increaseWaitFactor);
					} else {
						Log.error("Still no result after {} trials, giving up.", trial);
						done = true;
					}
				} else {
					Log.error("Non-recoverable failure, giving up.");
					done = true;
				}
			}
		}
	...
	public <R> Optional<R> withOptionalSupplier(ThrowingSupplier<Optional<R>> supplier) {
		...
		while(!done) {
			trial++;
			Log.debug("Doing trial {}", trial);
			result = Log.onException(supplier).warn("Exception in trial {}", trial).fallback(Optional.empty());
			done = result.isPresent();
			if (!done) {
				if (trial <= maxNumRetries) {
					Log.warn("Failure in trial #{}, sleeping for {}ms.", trial, wait);
					long thisWait = wait;
					Log.interruptable(() -> Thread.sleep(thisWait));
					wait = Math.round(wait * increaseWaitFactor);
				} else {
					Log.error("Got no result in trial #{}, giving up.", trial);
					done = true;
				}
			}
		}
