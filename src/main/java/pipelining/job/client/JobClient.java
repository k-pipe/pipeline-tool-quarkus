...
import pipelining.job.specification.JobDefinition;
import pipelining.http.Http;
import pipelining.job.status.*;
...
public class JobClient {
	...
	private boolean download(String inputDir, String filename, String dataUrl) {
		...
		log("Downloading {} from {}", file, dataUrl);
		boolean res = Http.download(dataUrl, file.toPath());
		if (res) {
			log("Successfully downloaded {} from {}", file, dataUrl);
		} else {
